class BugMix15 {
    ...
    public static void main(String[] args) {
        cache.add(new Object()); // Bug 1: Memory Leak
        new Thread(() -> {synchronized(A){try{Thread.sleep(10);}catch(Exception e){} synchronized(B){}}}).start();
        new Thread(() -> {synchronized(B){synchronized(A){}}}).start(); // Bug 2: Deadlock
        Integer max = Integer.MAX_VALUE;
        max = max + Integer.MAX_VALUE; // Bug 3: Integer overflow
        Object wrong = "str";
        int x = (int) wrong; // Bug 4: ClassCastException
    }
}
