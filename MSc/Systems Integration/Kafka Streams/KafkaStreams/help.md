# Cinema Streaming Platform — IS/EAI 2025/26
...
```bash
cd ~/Documents/GitHub/KafkaStreams
mvn clean package -q
cd .devcontainer
```
...
```bash
docker compose -f docker-compose-cluster.yml exec -T database psql -U postgres -d project3 < ../sql/create_tables.sql
```
...
```bash
curl -s http://localhost:7777/stats/revenue-per-film | jq .
curl -s http://localhost:7777/stats/profit-per-film | jq .
curl -s http://localhost:7777/stats/highest-profit-film | jq .
curl -s http://localhost:7777/genres | jq .
```
...
```bash
# Ver brokers ativos
docker compose -f docker-compose-cluster.yml ps | grep broker
